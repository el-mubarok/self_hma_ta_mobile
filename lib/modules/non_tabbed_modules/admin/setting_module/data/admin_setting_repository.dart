...
class AdminSettingRepository {
  ...
  Future<SharedBillingListData?> getListBilling([bool detail = false]) async {
    String path = AppApiRoutes.pathGetBilling;
    SharedBillingListData userList;
    Map<String, dynamic>? qParams = {};

    if (detail) {
      qParams.addAll({'detail': true});
    }

    try {
      Response response = await http.get(path, queryParameters: qParams);
  ...
  Future<SharedBillingData?> getBilling(
    dynamic id, [
    bool detail = false,
  ]) async {
